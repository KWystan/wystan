...
const socialIcons = {
  GitHub: (
    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" style={{ color: '#333' }}>
  ...
  Instagram: (
    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" style={{ color: '#E4405F' }}>
...
function TypewriterText() {
  ...
  return (
    <>
      {text}
      <span className="animate-blink text-black/55 font-light">|</span>
    </>
  );
}
...
export default function Hero() {
  ...
  // Compute real stats from portfolio data
  const stats = [
    { value: projects.length, label: 'Projects Built' },
    { value: new Date().getFullYear() - parseInt(experience[0]?.startDate?.split(' ')[1] || '2023'), label: 'Years Coding' },
